#!/usr/bin/env python3
"""
轮询拉取云端 ComfyUI 输出目录到本地 rock3/，保持远端文件名。
- 下载先写 .tmp 再原子改名，播放器永远读不到写了一半的文件
- 启动时以本地已有文件为基准，不重复下载
- SSH 连接复用（ControlMaster），降低轮询握手开销
启动：python3 sync_remote3.py
"""
...
REMOTE_DIR = "/root/ComfyUI/output/8.7/rock3"

LOCAL_DIR = Path(__file__).parent / "rock3"
POLL_INTERVAL = 1.0
...
SSH_OPTS = [
    ...
    "-o", "ControlPath=/tmp/rock-sync-rock3-%r@%h:%p",
    "-o", "ControlPersist=60",
]
